...
void loop()
{
  ...
  myDac.writeDac(2048, MCP4725_POWER_DOWN_MODE_100K); // Note, it doesn't matter what value we write here, the DAC will stop performing conversions
  delay(2000);

  Serial.println("Writing to the DAC in Power Down Mode (will not output conversion)...");
  myDac.writeDac(4095, MCP4725_POWER_DOWN_MODE_100K); // We should see no change in output voltage, as the DAC is still in power down mode

  delay(2000);

  // Now by issuing a writeDac command with normal mode enabled, we will wake the DAC from power down mode and again will see the value change
